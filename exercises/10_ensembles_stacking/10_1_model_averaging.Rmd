...
subtitle: "In-class exercise 10-1"
output:
...
## Exercise 1: Build a "Random Forest" From Scratch
...
Now reset the maximum depth hyperparameter values for each tree of your ensemble and the decision tree.
...
<details>
  ...
  You can then greplicate this part three times to create an actual graph and combine the output of the decision trees via the classifavg `PipeOp`.
  To plot the trees, the `rpart.plot` package is helpful.
  Regarding benchmarking, prior lectures should be helpful.
</details>
...
## Exercise 3: Optimizing Weights

Building upon the previous exercise, we now want to numerically optimize the weights of the ensemble via `bbotk`.
To do so, we will have to construct an `OptimInstance` in which we pass a domain, a search space, a codomain and the actual objective function that is optimized.
...
codomain = ps(classif.auc = ..., tags = "maximize"))
...
## Exercise 4: Optimizing Weights Efficiently
...
Construct an `OptimInstance` and optimize it via CMA-ES and terminate after 100 function evaluations.
