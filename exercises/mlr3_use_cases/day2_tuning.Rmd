...
The tuning instance is then defined by calling `TuningInstanceSingleCrit$new()`. 
...
This adjust the parameter set (`lrn_knn$param_set`) attached to the learner and flags it as "tunable". The tuning then makes use of that search space if no `search_space` is given in the `TuningInstanceSingleCrit$new()` construction:
...
<details>
<summary>**Click me**</summary>
```{r, fig.width=15, fig.height=5, out.width="100%", eval = show.solution}
...
- Based on this search space, we defined a tuning strategy to try 40 random configurations.
