...
subtitle: "Train, predict, evaluate with mlr3"
output:
...
On the other hand, (interest rates for) loans is an important revenue stream for banks.
...
This is were supervised learning models come into play.
...
Later, we will use the training data to learn a ML model and use the test data 
...
Install and loaded the `mlr3verse` package which is a collection of multiple add-on packages in the `mlr3` universe (if you fail installing `mlr3verse`, try to install and load only the `mlr3` and `mlr3learners` packages). 
...
Use either `TaskClassif$new()` or `as_task_classif()`.
...
```{r, eval=FALSE}
library(mlr3verse)
task = TaskClassif$new("german_credit", backend = ..., target = ..., positive = ...)
```
...
it is time to train your first supervised learning method.
...
Use `lrn()` or `LearnerClassif$new()` to initialize a `Learner` object.
...
Have a look on the coefficients by using `summary()`.
Name at least two features which have a significant effect on the outcome.
...
According to the summary, e.g., `credit_history` and `status` significantly influence the
creditworthiness and the bank's risk assessment.
By looking on `task$positive`, we could see that the class `good` (creditworthy client) is the positive class.
...
By using the `$score()` method, we obtain a estimate for the classification error
of our model.
