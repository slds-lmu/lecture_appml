...
* https://mlr3book.mlr-org.com/03-perf-binary.html

# German Credit Dataset

As in previous exercises, we use the German credit data of Prof. Dr. Hans Hoffman of the
...
## Training a classification tree on the german credit task
...
## Confusion matrices and classification thresholds
...
To do so, create predictions for all thresholds 0, 0.01, 0.02, ..., 1 and find the threshold that
minimizes this measure. Summarize the evaluated scores for these thresholds in a plot.
...
#   * prediction object
#   * threshold value (scalar value between 0 and 1)
#   * measure object
#   * task object
# It then gets new predictions for the given threshold and calculates the score of the specified measure.
# Note that it modifies the prediction in place.
...
# Example of the usage of `eval_threshold` with the FPR measure
eval_threshold(pred, threshold = 0.5, measure = msr("classif.fpr"), task = task)
eval_threshold(pred, threshold = 0.7, measure = msr("classif.fpr"), task = task)
```

<details>
<summary>**Hint 1:**</summary>
Read the examples of `msr("classif.costs")$help()` to learn how to define the cost function.
Use `sapply()` (or a for loop) in combination with the `eval_threshold` function above to compute the misclassification costs for the sequence of different threshold values.
You can use `which.min` to find the index of a vector with the minimum value.
</details>
...
```{r, eval = FALSE}
library(ggplot2)
costs = matrix(c(...), nrow = 2)
dimnames(costs) = list(truth = ..., response = ...)
measure = msr(...)
scores = sapply(thresholds, function(threshold) eval_threshold(...))

thresholds[which.min(...)]
min(...)
qplot(x = ..., y = ...)
```
</details>
...
We start with defining the thresholds we want to evaluate and the function that
returns the metric for each threshold.
...
We then define the cost matrix and from it the cost function.
...
```
Finally we evaluate each threshold, find the minimum and plot the results.
