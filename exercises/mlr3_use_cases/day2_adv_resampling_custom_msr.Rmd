...
subtitle: "Advanced Resampling and Custom Measures"
output:
...
Define you own risk measure for regression, the maximum of MSE and MAE: $f(y, \hat{y}) = \frac{1}{n} \sum_{i=1}^n \max((y_i - \hat{y}_i)^2,|y_i - \hat{y}_i|)$, using the code skeleton supplied above. Add it to the `mlr3measures` dictionary and use it to evaluate the following model prediction:
...
```
Now, add to dictionary and evaluate on predictions:
