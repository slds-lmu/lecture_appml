...
**Task: Construct a more refined initial design, using `paradox` to implement a Sobol design with 30 points, which has better coverage properties than grid or random search.**
...
An acquisition function relies on the prediction of a surrogate model and quantifies the expected ‘utility’ of each point of the search space if it were to be evaluated in the next iteration.
