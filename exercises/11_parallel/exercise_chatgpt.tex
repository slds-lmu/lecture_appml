...
\begin{enumerate}
    ...
    task = as_task_classif(oml_task$data)
    \end{lstlisting}
    ...
    addAlgorithm("learner", fun = function(job, data, instance, learner) {
      learner$train(data)
      learner$predict(data)$score()
    })
    \end{lstlisting}
    ...
    algo_design = list(learner = expand.grid(learner = learners))
