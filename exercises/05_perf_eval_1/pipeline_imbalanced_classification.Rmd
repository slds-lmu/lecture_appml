...
performance metric and learn about methods to adjust the standard machine learning models
to improve with respect to that metric.
...
## Benchmarking standard algorithms
...
Stratification consists of dividing the population into subsets (called strata)
within each of which an independent sample is selected. When setting the column role `"stratum"`,
resamplings that are applied to the task will automatically take the set stratum into account.

For binary classification problems, the column role `"positive"` is important, because metrics
like the TPR and FPR can only be understood when knowing what is defined as the positive class.
</details>

<details>
  <summary>**Hint 1:**</summary>
  The function `as_task_classif()` can help to create a classification task from the dataset.
  You can set the positive class and the stratification role by changing
  `task$positive` and `task$col_roles$stratum` after creating the task.

  Then, create the benchmark design using `benchmark_grid()` and execute it using
  `benchmark()`. To calculate the performance metrics use the method `$aggregate()`, which takes
  a list of measures that can be constructed using `msrs()`.
...
Now we conduct the benchmark experiment and compare the resulting predictions sing the accuracy,
FPR and TPR.

```{r}
learners = lrns(c("classif.rpart", "classif.ranger"))
design = benchmark_grid(task, learners, rsmp("repeated_cv", folds = 3, repeats = 5))

bmr = benchmark(design)
bmr$aggregate(msrs(c("classif.acc", "classif.fpr", "classif.tpr")))[, -2]
```

The accuracy is problematic for this problem, because it focuses too heavily on the majority class.
...
## Selecting a suitable performance metric

When selecting a suitable performance metric, we have to take the properties of
the task we are trying to solve into consideration. The detection of a spill
...
definition of the accuracy so it does not depend on the distribution of target
variable? Compare this new metric with the standard accuracy used in the previous exercise.

<details>
  <summary>**Hint 1:**</summary>
  The accuracy can be defined as
  $$ACC = P(\hat{Y} = 1 | Y = 1) \times P(Y = 1) + P(\hat{Y} = 0 | Y = 0) \times P(Y = 0)$$
  ...
  The new metric should **not** depend on the class distribution $P(Y = 1)$ (and $P(Y = 0) = 1 - P(Y = 1)$). That is we can equally weight the TPR and TNR to obtain a metric that does not take into account the class distribution. The resulting metric is known as the balanced
  accuracy: $BACC = 0.5 \cdot TPR + 0.5 \cdot TNR = 0.5 \cdot TPR + 0.5 \cdot (1-FPR)$
...
As defined in the hint, we have the accuracy
$$ACC = P(\hat{Y} = 1 | Y = 1) \times P(Y = 1) + P(\hat{Y} = 0 | Y = 0) \times P(Y = 0)$$
We can remove the effects of the class distribution by dividing the first term by $P(Y = 1)$ and the second
term by $P(Y = 0)$. When dividing the resulting expression by 2, we get what is referred to as the balanced accuracy (BACC).
$$BACC = 0.5 \times (P(\hat{Y} = 1 | Y = 1) + P(\hat{Y} = 0 | Y = 0))$$
...
<details>
  <summary>**Hint 1:**</summary>
  Use `po("classbalancing")` and combine it with the learner. PipeOps can be chained to a
  graph using `%>>%`. You can convert a graph to a learner using `as_learner()`
  Remember to use the instantiated resampling from the previous benchmark
  experiment. A new resample result can be added to a benchmark result using
  `c(...)`.
</details>
...
<details>
  ...
  graph = po("classbalancing", ratio = ..., reference = ..., adjust = ...) %>>%
    lrn(...)
...
```{r}
graph = po("classbalancing", ratio = 2.0, reference = "minor", adjust = "minor") %>>%
  lrn("classif.ranger")

graph$plot()
```

The instantiated resampling can be retrieved from the design. We can select any row, because
all instantiated resamplings from the previous benchmark experiment are identical.
...
This time we compare the balanced accuracies using a boxplot. The individual data-points for
each boxplot are folds of the repeated crossvalidation.

```{r}
autoplot(bmr, measure = msr("classif.bacc"))
```

Although the upsampling approach is superior to the vanilla random forest, there is still room
for improvement
...
Repeat the previous experiment, but now not only upsample the minority class like in the
previous exercise, but also downsample the majority class to the same count.
...
```{r}
graph = po("classbalancing", ratio = 2, reference = "minor", adjust = "all") %>>%
  lrn("classif.ranger")
...
Downsampling the majority class improved the result even further and is our best model so far.
...
## Bonus: Tuning the sampling rate
