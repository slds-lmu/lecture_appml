...
subtitle: "In-class exercise 02-2"
output:
...
As a follow-up on the tuning use case, we continue with the German credit task (`tsk("german_credit")`) and perform nested resampling. The purpose is to get a valid estimation of the generalisation error without an optimistic bias introduced by tuning without nested resampling. For this task, we estimate the generalisation error of a k-NN model implemented in `kknn`.

<details>
<summary>**Recap**</summary>
Nested resampling evaluates a learner combined with a tuning strategy to correctly estimate the generalisation error. Linking a tuning strategy with a learner uses the training data to find a good hyperparameter configuration (HPC). Using the same data for performance estimation and hence taking the best-estimated generalisation error of the best-performing model leads to an over-optimistic estimation. This is because information on the resampling splits may favour specific HPCs (overtuning) by chance.
</details>

## The AutoTuner

For this exercise, we use the same setup as for the tuning use-case with a 3-fold CV, `msr("classif.ce")` as performance measure, a random search, and a termination combination of 40 evaluations. Define an `AutoTuner` with `AutoTuner$new()` and train it on the `german_credit` task:
...
The `AutoTuner` is defined by `AutoTuner$new()` by specifying the `learner`, `resampling`, `measure`, `terminator`, `search_space`, and the `tuner`. The `AutoTuner` then behaves like a normal learner. We can use `$train` to fit the `AutoTuner` (tuning + model fit on best HPC) with the `Autotuner`.
</details>
...
The fitted learner on the best parameter configuration is stored in `at$model$learner$model`:
```{r, eval = show.solution}
at$model$learner$model
```
...
The estimated generalisation error then can be obtained by calling `$aggregate()` with the specific measure. The results from the individual folds can be obtained with `$score()`:
```{r, eval = show.solution}
...
bmr = benchmark(benchmark_grid(
  ...
  resamplings = outer))
```
Again, we can calculate the generalisation error with `$aggregate()` or visualize the result with `mlr3viz` and the `autoplot()` function:
```{r, eval = show.solution}
