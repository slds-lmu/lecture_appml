...
subtitle: "In-class exercise 01-2"
output:
...
<details>
  <summary>**Show Hint 2:**</summary>
  The key ingredients for `resample()` are a task (created by `TaskRegr$new()` or `tsk()`), a learner (created by `lrn()`) and a resampling strategy (created by `rsmp()`), e.g.,
