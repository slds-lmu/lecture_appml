...
subtitle: "Homework exercise Imputation"
output:
...
In this exercise, we want to compare the performance of two learners to impute the missing feature `temp`: a linear model and a k-nearest neighbors learner. For the k-nearest neighbor, we want to tune the hyperparameter `k` and compare how it affects the performance of a pipeline that connects the imputation with a random forest learner.
...
2) the `k` parameter of the kNN-based imputation method.
...
Create a task for the bike rental data and tune a graph learner with grid search over the defined search space, using 4-fold CV repeated 3 times, then visualize and interpret the results.
...
As can be seen, kNN imputation performs much better than LM-based imputation. For the `k` parameter, there is no clear trend, but values of 1, 5 and 8 seem to perform better.
