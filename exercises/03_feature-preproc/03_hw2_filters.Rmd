...
subtitle: "Homework exercise 03-2"
output:
...
Obviously, `waterfront` is a low-cardinality feature suitable for dummy (also called treatment) encoding and `zipcode` is a very high-cardinality feature. Therefore, it would make sense to create a pipeline that first pre-processes each factor variable with either impact or treatment encoding, depending on the feature cardinality.
...
1. Each factor variable gets pre-processed with either treatment or impact encoding, depending on the cardinality of the feature.
...
factor_po = po("encode", method = "treatment",
...
An alternative filter method is information gain (include source here). Recreate the pipeline from exercise 1, but use information gain as filter. Again, select the three features with the highest information gain. Train the `autotuner` on the training data, and evaluate the performance on the holdout test data.
...
Compare the performance between both filter methods. 
