...
```{r}
train = xgb.DMatrix(as.matrix(task$data(cols = task$feature_names)[split$train, ]),
                    label = ylab[split$train])
eval = xgb.DMatrix(as.matrix(task$data(cols = task$feature_names)[split$test, ]),
                   label = ylab[split$test])
```
