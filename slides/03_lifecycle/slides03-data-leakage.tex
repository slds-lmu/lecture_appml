...
\centering\footnotesize\itshape Each of the five types gets a worked example next.
...
% =====================================================================
% Frames 9a, 9b, 9c (new): Feature-selection leakage as a null simulation
% =====================================================================

% --- Frame 9a: Setup ---------------------------------------------------
\begin{frame}{Feature-selection leakage: null simulation}
\phantomsection\label{leakage-feature-selection-setup}

A worked example with \emph{no true signal} -- so we know exactly what an honest CV \emph{should} report.
...
\begin{itemize}
  ...
  \item $y$: exactly balanced binary target (50 of each class, then shuffled).
  \item $X$: $n = 100$ rows, $p = 10{,}000$ features, all i.i.d.\ standard normal.
  \item Every feature is \emph{independent of} $y$ by construction.
  \item Filter to top $k = 50$ features by univariate $F$-statistic.
  \item Fit a linear SVM; evaluate with 5-fold stratified CV; average over 50 seeds.
\end{itemize}

\medskip

\textbf{Expected outcome.} An honest CV should report accuracy near $\boldsymbol{0.50}$ -- there is no signal to learn.

\medskip

\centering\itshape Any clearly above-chance result must come from leakage, not real predictive structure.
\end{frame}


% --- Frame 9b: Two workflows ------------------------------------------
\begin{frame}{Feature-selection leakage: two workflows}
\phantomsection\label{leakage-feature-selection-workflows}

Run the same simulated data through two pipelines that differ only in \emph{when} feature filtering happens.
...
\textbf{\textcolor{leakred}{Leaky workflow}}
...
\begin{enumerate}
  ...
  \item Compute feature--target $F$-statistics on the \emph{full dataset}.
  \item Keep the top $k = 50$ features.
  \item Run 5-fold CV on the already-filtered data.
\end{enumerate}
\end{minipage}}

\smallskip
\textcolor{leakred}{\itshape\footnotesize Held-out labels influenced which features got selected.}
\end{column}

\begin{column}{0.49\textwidth}
\textbf{\textcolor{cleangreen!50!black}{Correct workflow}}
...
\begin{enumerate}
  \setlength\itemsep{0.2em}
  \item Compute $F$-statistics on the \emph{training fold only}.
  \item Keep the top $k = 50$ features.
  \item Fit the model on those features.
  \item Evaluate on the held-out fold.
\end{enumerate}
\end{minipage}}

\smallskip
\textcolor{cleangreen!50!black}{\itshape\footnotesize Held-out labels are never used during selection.}
\end{column}
...
% --- Frame 9c: Results + message --------------------------------------
\begin{frame}{Feature-selection leakage: simulation result}
\phantomsection\label{leakage-feature-selection-results}

50 seeds of the null simulation:
...
\begin{tikzpicture}[font=\footnotesize]
  % Leaky bar (red, big)
  \fill[leakred!70]   (0, 0) rectangle (5.0, 0.4);
  \node[left]  at (-0.1, 0.2) {Leaky CV};
  \node[right] at (5.1, 0.2) {\textbf{0.95}};

  % Proper bar (green, small)
  \fill[cleangreen!70] (0, -0.8) rectangle (2.6, -0.4);
  \node[left]  at (-0.1, -0.6) {Proper CV};
  \node[right] at (5.1, -0.6) {\textbf{0.49}};

  \node[gray, anchor=west, font=\scriptsize\itshape] at (-1.6, -1.3) {(mean over 50 seeds; $n = 100$, $p = 10{,}000$, $k = 50$)};
\end{tikzpicture}
\end{center}

\medskip

\textbf{What just happened.} On data with no real signal, the leaky pipeline reports near-perfect accuracy. Even the \emph{worst} leaky run is $0.88$; the worst proper run is $0.38$.

\medskip

\textbf{Reading this.} The correct procedure does \emph{not} give ``better performance'' -- it gives a more \textbf{honest estimate} of the pipeline's actual generalisation.

\medskip

\textbf{Mechanism = preprocessing.} The selector is a transformer fit on data. \textbf{Fix:} re-rank inside each CV fold; wrappers that tune their own selection require nested resampling.
\end{frame}


% =====================================================================
% Frame 10: The Golden Rule (centrepiece) -- formerly frame 9 in v1; now follows the 3-frame feature-selection example
% =====================================================================
...
% =====================================================================
% Frame 11: Spotting leakage -- heuristics + audit checklist (merged)
% =====================================================================
