...
\centering\footnotesize\itshape Each of the five types gets a worked-example frame next.
...
% =====================================================================
% Frame 9 (new): Worked example -- Feature-selection leakage
% =====================================================================
\begin{frame}{Worked example: feature-selection leakage}
\phantomsection\label{leakage-feature-selection}

\textbf{Scenario.} Rank features by their correlation with the target, keep the top $k$, fit a model, report 10-fold CV accuracy.

\medskip

\begin{center}
\begin{tikzpicture}[font=\footnotesize]
  % Reported CV (leaky) bar
  \fill[leakred!70]   (0, 0) rectangle (5.0, 0.4);
  \node[left]  at (-0.1, 0.2) {Reported CV};
  \node[right] at (5.1, 0.2) {\textbf{0.92}};

  % Clean held-out test bar
  \fill[cleangreen!70] (0, -0.8) rectangle (3.0, -0.4);
  \node[left]  at (-0.1, -0.6) {Clean held-out test};
  \node[right] at (5.1, -0.6) {\textbf{0.55}};

  \node[gray, anchor=west, font=\scriptsize\itshape] at (-1.3, -1.3) {(illustrative numbers)};
\end{tikzpicture}
\end{center}

\medskip

\textbf{The catch.} The top-$k$ features were chosen using \emph{all} of $y$ -- including labels in the held-out fold. Those labels had already influenced which features made it into the model.

\medskip

\textbf{Same mechanism as preprocessing.} The selector is a transformer fit on data, same as a scaler or encoder. Same failure mode, same fix.

\medskip

\textbf{Fix.} Re-rank features \emph{inside each CV fold}. Wrappers that tune their own selection require nested resampling.
\end{frame}


% =====================================================================
% Frame 10: The Golden Rule (centrepiece)
% =====================================================================
