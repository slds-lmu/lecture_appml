...
\begin{frame}{Loss Function versus Performance Measure}
...
\begin{itemize}
    \itemsep1em
    \item LFs are used to optimize model parameters \textbf{during training}. PMs are used to evaluate the model performance \textbf{after training} on unseen test data.
    ...
    \item We can use the same LF and PM. However, one typically selects the PM based on the prediction task and opts to approximate this metric with an LF that is numerically easier to handle.
...
\begin{itemize}
    ...
    \item \textbf{Regression:}
        \begin{itemize} 
            \itemsep1em
            \item We often optimize the mean squared error (MSE) during training (LF), which can also be used to evaluate various final model candidates (PM).
...
\begin{itemize}
    ...
    \begin{figure}
        \centering
        \includegraphics[width = 0.8\textwidth]{slides/04-perf-eval/figure/metrics_task_overview.png}
    \end{figure}
\end{itemize}
...
\begin{frame}{Overview for Classification}
    ...
    \begin{figure}
        \centering
        \includegraphics[width = 0.9\textwidth]{slides/04-perf-eval/figure/classification_measures.png}
    \end{figure}
