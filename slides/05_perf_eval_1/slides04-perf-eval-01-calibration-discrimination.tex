...
\includegraphics{slides/05_perf_eval_1/figure_man/calibplot2.png}
...
Permuting predictions (e.g., randomly assigning different predictions to observations) within each group (here: decile)
...
\textbf{Goal}: Test if a classifier is really uncalibrated on the given data set
...
\begin{itemize}
    ...
    \item $O_m^+$ observed positives or $O_m^-$ observed negatives per class
    \item $E_m^+$ expected positives or $E_m^-$ expected negatives
\end{itemize}

if $H$ is small, the observed outcomes match the predicted probabilities well, i.e. model is well-calibrated

\item this follows a Chi-Squared distribution with $M-2$ degrees of freedom. Calculate p-value on this
\end{itemize}
...
\includegraphics{slides/05_perf_eval_1/figure_man/OverAndUnderConfidence.png}
\citebutton{Filho et al. (2023).}{https://link.springer.com/article/10.1007/s10994-023-06336-7/figures/3}

\begin{itemize}
    \item \textbf{Underconfidence}: the dots should move outwards on the x-axis (i.e. the classifier should be more confident)
    \item \textbf{Overconfidence}: the dots should move inwards on the x-axis (i.e. the classifier should be less confident)
\end{itemize}
...
\emph{Empirical binning} partitions the scores \( s \in \mathbb{S}\) into bins \( B_1, \ldots, B_M \) and defines a (piecewise constant) calibration function \( C \) by:
...
\emph{Platt scaling} applies logistic regression to predicted scores $s \in \mathbb{S}$, i.e., it fits a calibration function $C$ minimizing the log-loss on $\mathcal{D}_{cal}$ by:
  ...
  \begin{itemize}
    ...
    \item requires: unbounded outputs before sigmoid/ softmax, good for Neuronal Networks, SVM, boosted trees
\end{itemize}
  \vfill\centering
...
\begin{frame}[t]
	\frametitle{Isotonic Regression - PAVA}
	 PAVA iterates the following steps (simplified to avoid notational overload):
		...
		\begin{itemize}
			\item[(1)] Find first violating adjacent bins $B_i$ and $B_{i+1}$ such that $C(B_i) > C(B_{i+1})$.
			\item[(2)] Merge $B_i$ and $B_{i+1}$ into a new bin $B$. Stop if no violation occured.
			\item[(3)] If $C(B) < C(B_{i-1})$ for the left neighbor bin $B_{i-1}$, merge also these bins and continue until no more violations are found (monotonicity).
			\item[(4)] Continue with (1).
		\end{itemize}
	
	\begin{center}
		\includegraphics[width=0.65\textwidth]{figure/pic-pava}
	\end{center}
\end{frame}
...
\includegraphics[width = \linewidth]{slides/05_perf_eval_1/figure_man/CalibWorkflow1.png}
...
\includegraphics[width = \linewidth]{slides/05_perf_eval_1/figure_man/CalibWorkflow2.png}

Disadvantage:
\begin{itemize}
    \item we fit multiple models and calibrators, which takes long time
    \item non of the calibrators is fitted on all data
\end{itemize}
...
\begin{frame}{Calibration Workflow}
\textbf{Option B}: Cross-validation poolded
\includegraphics[width = 0.7 \linewidth]{slides/05_perf_eval_1/figure_man/CalibWorkflow3.png}
\begin{itemize}
    \item fit multiple models on CV folds
    \item fit calibrator on pooled out-of-fold predictions and data
    \item fit one final model on all data
    \item put new data point through final model and calibrator (only one model and calibrator, fitted on all data)
\end{itemize}
\end{frame}
...
\includegraphics[width = 0.7 \linewidth]{slides/05_perf_eval_1/figure_man/CalibrationCVFold.png}
