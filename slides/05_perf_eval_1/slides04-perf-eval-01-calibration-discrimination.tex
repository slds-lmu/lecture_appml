...
\begin{itemize}
    \item Stakeholders must understand metrics relevant to the use case.
...
\begin{itemize}
  ...
  \item<2-> \textbf{Calibration:} When the predicted probabilities $\hat{p}$ closely agree with the observed proportion for $Y = 1$ (for any reasonable grouping).
  ...
  % \only<2-3> {\begin{itemize}
     \only<3-> {\item[$\Rightarrow$] \textbf{Calibration in the large:} %is a property of the \textit{full sample}.\\
  ...
  %     \includegraphics[page=3, width=0.85\textwidth]{figure_man/calibration_large_vs_small.pdf}}
     \visible<4> {\item[$\Rightarrow$] \textbf{Calibration in the small:} %is a property of \textit{subsets} of the sample.\\
  ...
  \only<3> {\centering\includegraphics[page=3, width=0.75\textwidth]{figure_man/calibration_large_vs_small.pdf}}
  \only<4> {\centering\includegraphics[page=4, width=0.75\textwidth]{figure_man/calibration_large_vs_small.pdf}}
\end{itemize}
\end{frame}
