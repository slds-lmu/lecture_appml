...
\begin{itemize}
  \item \textbf{Discrimination:} Ability of $\hat Y$ to well-separate positive/negative instances.\\
  $\Rightarrow$ Confusion matrix-based measures are discrimination measures, e.g.,
   $FPR = P(\hat Y = 1 | Y = 0$), $TPR = P(\hat Y = 1 | Y = 1)$, AUC.
  \item \textbf{Calibration:} When the predicted probabilities $\hat{p}$ closely agree with the observed outcome $Y$ (for any reasonable grouping).
  ...
  %   $\Rightarrow$ Compares observed probability (e.g. proportion of positive instances)
  %  % <!-- (e.g., 10% if 10 of 100 individuals have the outcome being predicted, e.g. $y=1$) -->
  %   with the average predicted probability in the \textit{full sample}.
  %   \item \textbf{Calibration in the small} is a property of \textit{subsets} of the sample.\\
  %   $\Rightarrow$ Compares observed proportion of positive instances  with the average predicted probability in each subset (often constructed by deciles).
  % \end{itemize}
\end{itemize}
...
\begin{frame}{Calibration in the Large}
    ...
    \makebox[\linewidth]{\includegraphics[page=1, width=\textwidth]{figure/calibration_large_vs_small.pdf}}
\end{frame}
...
\begin{itemize}
  \item Both models are well discriminating, i.e., setting thresholds $c_1 = 0.5$ for $\fh_1$ and $c_2 = 0.8$ for $\fh_2$ perfectly separates positive and negative observations.
...
\begin{itemize}
\item Monotonic transformations of the predicted probabilities (e.g., $\tfrac{\hat p + 0.2}{1.2}$) do not change the ROC curve as the ranking will not change.
\item The calibration plot looks worse as the probabilities are changed.
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Permuting the predictions (e.g., randomly assigning different predictions to observations) within each decile does not change the calibration plot.
\item However, the ROC curve and AUC can get worse as the ranking within each decile is changed.
\end{itemize}
