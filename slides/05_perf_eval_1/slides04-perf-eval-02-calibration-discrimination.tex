...
\begin{itemize}
  ...
  \only<2> {\centering\includegraphics[page=3, width=0.75\textwidth]{figure_man/calibration_large_vs_small.pdf}}
  \only<3> {\centering\includegraphics[page=4, width=0.75\textwidth]{figure_man/calibration_large_vs_small.pdf}}
\end{itemize}
\end{frame}
...
\textbf{Gap pred. mean:} Shows deviation between average predicted probability (red) and observed proportion of positives (blue) within each group.
...
\begin{itemize}
    \item The closer the points to the diagonal line, the better calibration
...
\begin{itemize}
  ...
  \item Calibration should be performed on new data not used for model fitting.
\end{itemize}
