...
\newcommand{\titlefigure}{figure/gaussian_differences.png}
...
\large{Two algorithms on 1 data set}
    \begin{figure}
        \includegraphics[width = 0.6\textwidth]{slides/04-perf-eval/figure/2algos_1dataset.png}
    \end{figure}
\vfill
\end{frame}
...
\begin{frame}{Two-matched-samples t-test}
    ...
    \begin{itemize}
        \item Given a holdout test set $\Dtest$, can we assume a difference in performance between $\widehat{f}_1$ and $\widehat{f}_2$ generalizes to other data?
        $$H_0: \widehat{\text{GE}}(\widehat{f}_1, L, \Dtest) = \widehat{\text{GE}}(\widehat{f}_2, L, \Dtest) \text{ vs. } H_1: \widehat{\text{GE}}(\widehat{f}_1, L, \Dtest) \neq \widehat{\text{GE}}(\widehat{f}_2, L, \Dtest)$$
        \item Test statistic: 
            $$
        ...
        $$
        $$d_i = L(\yi, \fh_1 (\xi)) - L(\yi, \fh_2 (\xi))\qquad \bar{d} = \sum\limits_{i=1}^{n} d_i \qquad (\xi, \yi) \in \Dtest$$
...
\begin{frame}{Two-Matched-Samples T-Test}
\begin{itemize}
    \item \textbf{Example:} We evaluate learners \texttt{classif.rpart} and \texttt{classif.ranger} on the binary classification task \texttt{strikes}.
    \item We use the logarithmic (also known as cross-entropy) loss:
    {
        \scriptsize
        \include{slides/04-perf-eval/rsrc/rpart_ranger_strikes_task}
    }
    \item The test statistic corresponds to:
    $$
    T_{\text{t-test}} = \sqrt{625} \; \frac{-0.0281}{0.3584} = -1.958
    $$
    \item For $\alpha = 0.05$, the critical values to reject $H_0$ are -1.9637 and 1.9637.
    \item For the given significance level, there is not enough evidence to reject $H_0$. We assume both algorithms perform equally well on this population of data.
\end{itemize}
...
%\hspace*{\fill}
  \begin{tabular}{cc|cc}
      ...
      & & correct & wrong \\
      \hline
      \multirow{2}{*}{\textbf{rpart}} & correct & 561 & 2 \\
      & wrong & 46 & 16 \\
  \end{tabular}
\end{center}
...
$$T_{\text{McNemar}} =  \frac{(|2 - 46| - 1)^2}{2 + 46} \approx 38.52 
$$
% > 3.841 = \chi^2_{1,0.95}.$$
\begin{itemize}
    ...
    \item The t-test performs on observation-wise loss values, while the McNemar test performs on aggregate classification errors. Both tests can have different results!
\end{itemize}
...
\large{Two algorithms on multiple data sets}
    \begin{figure}
        \includegraphics[width = 0.6\textwidth]{slides/04-perf-eval/figure/2algos_multipledatasets.png}
    \end{figure}
\vfill
\end{frame}
...
\begin{frame}{Wilcoxon Signed Rank Test}
    \begin{itemize}
        ...
        \item $H_0$: Model 1 and 2 have tied ranks in model performance. \\ 
        $H_1$: Model 1 and 2 have different ranks in performance.
        \item Consider $M$ data sets. Let $\widehat{\text{GE}}(\widehat{f}_k, \rho, \Dtestm)$  denote the GE estimate for model $k$ and data set $m$. The difference $d_b$ between GE estimates on the $m$-th data set is:
        $$
        d_m = \widehat{\text{GE}}(\widehat{f}_1, \rho, \Dtestm) - \widehat{\text{GE}}(\widehat{f}_2, \rho, \Dtestm)
        $$
        \item Let $R^{+}$ denote the rank sum for data sets where algorithm B outperforms algorithm A and vice versa for $R^{-}$:
        $$
...
\begin{frame}{Wilcoxon Signed Rank Test}
    ...
    {   
        \scriptsize
        \include{slides/04-perf-eval/rsrc/friedman_benchmark_results_rpart_ranger}
    }
...
% \end{table}
$$
    R^{+} = 4\qquad R^{-} = 1\qquad \min(R^{+}, R^{-}) = 1
    $$
    $$
    T_{\text{Wilcoxon}} = \frac{1 - \frac{1}{4}6(6 + 1)}{\sqrt{\frac{1}{24}6(6 + 1)(12 + 1)}} = - 1.99 
    $$
    \item For $\alpha = 0.05$, the critical value is 0.
    \item $T_{\text{Wilcoxon}} < 0 \Rightarrow$ reject $H_0$! 
...
\large{Multiple algorithms on multiple data sets}
    \begin{figure}
        \includegraphics[width = 0.6\textwidth]{slides/04-perf-eval/figure/multiplalgos_multipledatasets.png}
    \end{figure}
\vfill
\end{frame}
...
\begin{figure}
    \centering
    \includegraphics{slides/04-perf-eval/figure/omnibustesting.png}
    % \caption{Caption}
    % \label{fig:enter-label}
\end{figure}
...
{
    \tiny
    \include{slides/04-perf-eval/rsrc/friedman_benchmark_results_short}
}
    \centering
    \includegraphics[width = 0.65\textwidth]{slides/04-perf-eval/figure/benchmarkrankplot.png}

\end{frame}

% \begin{frame}{Friedman test}
%     \includegraphics[width = \textwidth]{slides/04-perf-eval/figure/benchmarkcolplot.png}
% \end{frame}

% \begin{frame}{Friedman test}
%     \includegraphics[width = 0.5\textwidth]{slides/04-perf-eval/figure/benchmarkrankplot.png}
% \end{frame}
...
$H_1: \bar{R}_{.k_1} \neq \bar{R}_{.k_2}$
\item The test statistic corresponds to the difference in average ranks:
$$
T_{\text{Nemenyi}} = \bar{R}_{.k_1} - \bar{R}_{.k_2} 
$$
with critical value to reject $H_0$:
$$ 
T_{\text{Nemenyi}} \geq q_{\alpha} \sqrt{\frac{K(K + 1)}{6M}}
$$
where $q_{\alpha}$ is based on the studentized range statistic divided by $\sqrt{2}$.
...
\begin{align*}
    ...
    T_{\text{Nemenyi}} &= 3.53 - 1.59 = 1.94
\end{align*}
\begin{itemize}
    \item For $\alpha = 0.05$, critical difference in average ranks is 2.84.
    \item $T_{\text{Nemenyi}} > 2.84 \Rightarrow$ Do not reject $H_0$!
    \item There is not enough evidence to reject the claim that on more data sets, ranger and rpart would have the same average rank.
\end{itemize}
    % \begin{itemize}
...
% \textbf{Post-hoc Bonferroni-Dunn test}:
    ...
    %Another difference between Bonferroni-Dunn Test with Nemenyi test is that total number of comparisons are 1/k of Nemenyi test.
     ...
     $$
     \item In our example, $\alpha_{\text{corrected}} = \frac{0.05}{5} = 0.01$. The difference in average ranks between rpart and ranger would need to be greater than 4.12 to reject $H_0$.
...
\begin{itemize}
    ...
    \item For instance, the difference in mean ranks between ranger and the SVM does not exceed the CD but the one between ranger and rpart does. 
\end{itemize}
\includegraphics[width = \textwidth]{slides/04-perf-eval/figure/crit_diff_plot.png}
\end{frame}
...
\begin{center}\includegraphics[width = 1\textwidth]{slides/04-perf-eval/figure/tests.pdf} \end{center}
