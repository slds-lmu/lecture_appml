...
\newcommand{\titlefigure}{slides/04-perf-eval/figure/gaussian_distribution}
...
$$T_{\text{McNemar}} =  \frac{(|5-17| - 1)^2}{5 + 17} = 5.5 
$$
% > 3.841 = \chi^2_{1,0.95}.$$
\begin{itemize}
    \item For $\alpha = 0.5$, the critical value is 3.841.
    \item $T_{\text{McNemar}} > 3.841 \Rightarrow$ Reject $H_0$.
    \item We conclude that the tree and random forest do not have the same performance.
\end{itemize}
...
\begin{itemize}
  ...
  $ SS_{Total} = M \sum_{k=1}^{K} (\bar{R}_{.k} - \bar{R})^2 $, where $\bar{R}_{.k} =  \frac{1}{M} \sum_{m=1}^{M} R_{mk}$.
  \item The error sum of squares:
  $ SS_{Error} = \frac{1}{M(K-1)} \sum_{m=1}^{M} \sum_{k=1}^{K} (R_{mk} - \bar{R})^2 $
\end{itemize}

The Friedman test statistic is calculated as:

$$T_{\text{Friedman}} = \frac{SS_{Total}}{SS_{Error}} \sim \chi_{K-1}^2 \text{ for large \textit{M} ($>15$) and \textit{K} ($>5$)}$$
\end{frame}
...
\begin{tabular}{rlrrrrrr}
 ...
 & task\_id & encode.classif.featureless & encode.classif.cv\_glmnet & encode.classif.rpart & encode.classif.ranger & encode.classif.kknn & encode.classif.svm \\ 
  \hline
1 & mfeat-morphological & 0.10 & 0.00 & 0.01 & 0.00 & 0.00 & 0.00 \\ 
  2 & banknote-authentication & 0.44 & 0.01 & 0.03 & 0.01 & 0.00 & 0.00 \\ 
  3 & socmob & 0.22 & 0.06 & 0.07 & 0.06 & 0.16 & 0.07 \\ 
  4 & space\_ga & 0.50 & 0.16 & 0.21 & 0.16 & 0.19 & 0.16 \\ 
  5 & delta\_elevators & 0.51 & 0.12 & 0.13 & 0.12 & 0.14 & 0.12 \\ 
  6 & analcatdata\_supreme & 0.24 & 0.01 & 0.01 & 0.01 & 0.02 & 0.02 \\ 
  7 & delta\_ailerons & 0.47 & 0.06 & 0.06 & 0.06 & 0.06 & 0.06 \\ 
  8 & pollen & 0.52 & 0.52 & 0.51 & 0.50 & 0.52 & 0.52 \\ 
  9 & fri\_c0\_1000\_5 & 0.53 & 0.15 & 0.18 & 0.11 & 0.13 & 0.10 \\ 
  10 & fri\_c1\_1000\_5 & 0.46 & 0.39 & 0.15 & 0.09 & 0.12 & 0.10 \\ 
  11 & phoneme & 0.29 & 0.26 & 0.17 & 0.10 & 0.12 & 0.16 \\ 
  12 & puma8NH & 0.50 & 0.17 & 0.20 & 0.17 & 0.22 & 0.17 \\ 
  13 & wilt & 0.05 & 0.04 & 0.02 & 0.02 & 0.03 & 0.02 \\ 
  14 & car & 0.30 & 0.05 & 0.04 & 0.03 & 0.17 & 0.02 \\ 
  15 & fri\_c2\_1000\_5 & 0.42 & 0.26 & 0.11 & 0.08 & 0.10 & 0.07 \\ 
  16 & kin8nm & 0.49 & 0.27 & 0.28 & 0.16 & 0.16 & 0.11 \\ 
   \hline
\end{tabular}
\end{table}
...
$$
\bar{R}_{.k} =\frac{1}{M} \sum_{m=1}^M R_{mk}
$$
...
\textbf{Example (for one pairwise comparison):}

\begin{align*}
    \bar{R}_{.2} &= \frac{1}{6} \sum_{m=1}^M R_{m_2} = \frac{13}{6} \\
    \bar{R}_{.3} &= \frac{1}{6} \sum_{m=1}^M R_{m_3} = \frac{17.5}{6} \\
    % T_{\text{Nemenyi}} &= \frac{\frac{13 - 17.5}{6}}{\sqrt{\frac{4(4+1)}{36}}} \approx - 1 
    T_{\text{Nemenyi}} &= \frac{-4.5}{6} = -0.75
\end{align*}
\begin{itemize}
    \item For $\alpha = 0.05$, critical difference in average ranks is 2.569.
    \item $T_{\text{Nemenyi}} < 2.569 \Rightarrow$ Do not reject $H_0$!
    \item We conclude the performance of algorithms 2 and 3 is equal.
\end{itemize}
    % \begin{itemize}
