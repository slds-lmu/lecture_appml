...
\newcommand{\Dtestb}{\mathcal{D}_{\text{test}}^{(b)}}
...
\begin{frame}{Two-matched-samples t-test}
    ...
    \begin{itemize}
        \setlength\itemsep{1em}
        \item Does model $\widehat{f}_1$ outperform $\widehat{f}_2$ on $ \Dtest$?
        ...
        \item Test statistic: 
            ...
            \sigma_{d} = \sqrt{\frac{1}{n_{test} - 1}\sum_{i=1}^{n_{test}} \left(d_i - \bar{d} \right)^2}
        $$
        ...
        \begin{itemize}
            \item \textbf{iid of loss values:} %Samples need to be iid samples from the underlying population 
...
\begin{frame}{Wilcoxon Signed Rank Test}
    \begin{itemize}
        ...
        \item Consider $B$ data sets. Let $\widehat{\text{GE}}(\widehat{f}_k, L, \Dtestb)$  denote the GE estimate for model $k$ and data set $b$. The difference $d_b$ between GE estimates on the $b$-th data set is:
        $$
        d_b = \widehat{\text{GE}}(\widehat{f}_1, L, \Dtestb) - \widehat{\text{GE}}(\widehat{f}_2, L, \Dtestb)
        $$
        \item Let $R^{+}$ denote the rank sum for data sets where algorithm B outperforms algorithm A and vice versa for $R^{-}$:
        $$
        R^{+} = \sum_{d_b > 0} \text{rank}(d_b) + \frac{1}{2} \sum_{d_b = 0} \text{rank}(d_b) \quad\quad R^{-} = \sum_{d_b < 0} \text{rank}(d_b) + \frac{1}{2} \sum_{d_b = 0} \text{rank}(d_b)
        $$
        ...
        $$
        T_{\text{Wilcoxon}} = \frac{\min(R^{+}, R^{-})   - \frac{1}{4}B(B + 1)}{\sqrt{\frac{1}{24}B(B + 1)(2B + 1)}}
        $$
    \end{itemize}
\end{frame}
...
Suppose we want to evaluate $B$ data sets and $K$ algorithms. The Friedman test first ranks algorithms for each data set separately:
...
\begin{itemize}
  ...
  \item Let $R_{bk}$ denote the rank of algorithm $k$ on data set $b$.
\end{itemize}
...
\begin{itemize}
  ...
  $ \bar{R} = \frac{1}{bk} \sum_{k=1}^{K} \sum_{b=1}^{B} R_{bk}$
  \item The total sum of squares:
  $ SS_{Total} = B \sum_{k=1}^{K} (\bar{R}_{.k} - \bar{R})^2 $, where $\bar{R}_{.k} =  \frac{1}{B} \sum_{b=1}^{B} R_{bk}$.
  \item The error sum of squares:
  $ SS_{Error} = \frac{1}{B(K-1)} \sum_{b=1}^{B} \sum_{k=1}^{K} (R_{bk} - \bar{R})^2 $
\end{itemize}

The Friedman test statistic is calculated as:

$$T_{\text{Friedman}} = \frac{SS_{Total}}{SS_{Error}} \sim \chi_{K-1}^2 \text{ for large \textit{B} ($>15$) and \textit{K} ($>5$)}$$
\end{frame}

\begin{frame}{Friedman Test}

\textbf{Illustrative example ($B < 15$ and $K < 5$ here, so test statistic not $\chi^2$-distributed):}
    ...
    \end{table}
\begin{align*}
    \bar{R} &= \frac{1}{6 \cdot 4} \sum_{k=1}^{4} \sum_{b=1}^{6} R_{bk} = \frac{60}{24} = 2.5 \\
    ...
    \\
    SS_{Error} &= \frac{1}{6(4-1)} \sum_{b=1}^{6} \sum_{k=1}^{4} (R_{bk} - \bar{R})^2 = 1.375 \\
...
\item Runs all $\frac{K(K-1)}{2}$ pairwise comparisons for $B$ data sets and $K$ algorithms.
\item Calculates the average rank of $k$-th algorithm on all $B$ data sets: 
$$
\bar{R}_{.k} =\frac{1}{B} \sum_{b=1}^B R_{bk}
$$
...
$$ 
T_{\text{Nemenyi}} \geq q_{\alpha} \sqrt{\frac{K(K + 1)}{6B}}
$$
...
\begin{align*}
    \bar{R}_{.2} &= \frac{1}{6} \sum_{b=1}^B R_{b2} = \frac{13}{6} \\
    \bar{R}_{.3} &= \frac{1}{6} \sum_{b=1}^B R_{b3} = \frac{17.5}{6} \\
...
% \textbf{Post-hoc Bonferroni-Dunn test}:
    ...
    $$
        T_{\text{Bonferroni-Dunn}} = \frac{\bar{R}_{.k_1} - \bar{R}_{.k_{\text{baseline}}}}{\sqrt{\frac{K(K+1)}{6B}}}
    $$
    % \item 
