...
\item We first rank each algorithm on each data set (the lower the rank in parantheses, the better the performance):
\end{itemize}
...
{
    \scriptsize
    \include{slides/04-perf-eval/rsrc/friedman_benchmark_results}
}
...
\begin{itemize}
  ...
  \begin{flalign*}
     & \bar{R} = \frac{1}{mk} \sum_{k=1}^{K} \sum_{m=1}^{M} R_{mk} 
      = \frac{1}{96} \sum_{k=1}^{6} \sum_{m=1}^{16} R_{mk} \approx 3.427 &
  \end{flalign*}
  \item The total sum of squares:
  \begin{flalign*}    
    & SS_{Total} = M \sum_{k=1}^{K} (\bar{R}_{.k} - \bar{R})^2 = 16 \sum_{k=1}^{6} (\bar{R}_{.k} - 3.427)^2 \approx 179.427 & \\ 
  ...
  \item The error sum of squares:
    \begin{flalign*}  
        & SS_{Error} = \frac{1}{M(K-1)} \sum_{m=1}^{M} \sum_{k=1}^{K} (R_{mk} - \bar{R})^2 = \frac{1}{80} \sum_{m=1}^{16} \sum_{k=1}^{6} (R_{mk} - 3.427)^2\approx 3.394 & 
    \end{flalign*}
\end{itemize}
...
$$T_{\text{Friedman}} = \frac{SS_{Total}}{SS_{Error}} = \frac{178.427}{3.427} \approx 52.872
$$
