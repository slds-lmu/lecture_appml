...
\begin{columns}
    \begin{column}{0.5\textwidth}
        \begin{figure}
            \centering
            \includegraphics[width=\textwidth]{figure_man/bagging_variance_bias_tree.png}
    ...
    \begin{column}{0.5\textwidth}
        ...
        \begin{itemize}
            \item Variance (green) is the dominant error component; bias$^2$ (blue) stays low
            \item Error peaks (red) coincide with regions of sharp change in $f(\xv)$, where individual trees vary most
            \item Top subplot: light red band shows the spread of individual tree fits $\hat{y}$ around $\mathbb{E}(\hat{y})$ (cyan)
        \end{itemize}
    \end{column}
\end{columns}
...
\vspace{-0.5em}
\footnotesize
Compare bottom subplots: variance (green) nearly vanishes with 100 trees $\rightarrow$ total error (red) drops dramatically. Bias (blue) stays roughly unchanged. Individual fits (light red band, top) tighten around $\mathbb{E}(\hat{y})$.\\
\vfill
Works well for unstable learners. NB: RF also uses feature (column) subsampling.
...
Iterative technique that sequentially trains subsequent base models to correct errors of previous models, for example, by re-weighting instances or predicting pseudo-residuals $\rightarrow$ homogeneous \& sequential ensemble
\vfill
Examples:
\begin{itemize}
...
\begin{itemize}
    \item $\fM = \text{mode}(\{\bll[1](\xv), \ldots, \bll[M](\xv)\})$ assuming hard label predictions
    \item $\fM = \frac{1}{M} \sum_{m = 1}^{M} \blp[m](\xv)$ assuming probability predictions
\end{itemize}
\vfill
...
\vfill
Assign weights $\mathbf{w} = (w_{1}, \ldots, w_{M}), \text{usually: } 0 \le w_{m} \le 1, \sum_{m=1}^{M} w_{m} = 1$ to the models\\
\vfill
Classification: $f_{\mathbf{w}}(\xv) = \sum_{m = 1}^{M} w_{m} \cdot \blp[m](\xv)$\\
\vfill
Regression: $f_{\mathbf{w}}(\xv) = \sum_{m = 1}^{M} w_{m} \cdot \blx[m]$\\
\begin{figure}
    \centering
...
Find the optimal weight vector $\mathbf{w}^{\star}$ such that the estimated generalization error (based on a performance metric $\rho$ or loss function $L$) of the ensemble is optimal:
$$\mathbf{w}^{\star} \coloneqq \operatorname*{arg\,min}_{\mathbf{w} \in \mathbb{R}^{M}} \hat{\text{GE}}\left(\hat{f}_{\mathbf{w}}, \JJ, \rho\right)$$
\vfill
...
\begin{itemize}
    \item MSE (regression) $L_{\mathrm{MSE}}(y^{(i)}, \hat{f}_{\mathbf{w}}(\mathbf{x}^{(i)})) = (y^{(i)} - \hat{f}_{\mathbf{w}}(\mathbf{x}^{(i)}))^{2}$;\\
          ...
          $$\operatorname*{arg\,min}_{\mathbf{w} \in \mathbb{R}^{M}} \hat{\text{GE}}(\hat{f}_{\mathbf{w}}, \JJ, L_{\mathrm{MSE}}) \text{ s.t. } 0 \le w_{m} \le 1, \sum_{m=1}^{M} w_{m} = 1$$
...
\begin{enumerate}
    ...
    \item Add the base model that maximizes the ensemble's performance metric on a hillclimb (validation) set
...
$\rightarrow$ $\mathbf{w}^{\star} = (\frac{4}{6}, \frac{1}{6}, \frac{1}{6})^\intercal$
\end{vbframe}
...
Model Averaging ($\mathbf{w} = (0.75, 0.05, 0.20)^\intercal)$ & 0.875 & 0.810 \\
Model Averaging (GES: $\mathbf{w}^{\star} = (\frac{4}{6}, \frac{1}{6}, \frac{1}{6})^\intercal$) & 0.917 & 0.864\\
\hline
\end{tabular}
...
\begin{itemize}
    \item \textbf{Statistical Problem:} Limited data $\rightarrow$ many models achieve similar training error; averaging reduces the risk of picking a bad one
    \item \textbf{Computational Problem:} Optimization can get stuck in local optima; combining multiple runs explores more of the hypothesis space
    \item \textbf{Representational Problem:} The true function may lie outside any single hypothesis space; a combination can better approximate it
\end{itemize}
\vfill
Requirements to build an ensemble:
\begin{itemize}
    \item \textbf{Diverse Models:} Base models should make different errors (complementary strengths)
    \begin{itemize}
        \item Use different algorithms
        \item Use different hyperparameter configurations of the same algorithm
        \item Use different feature subsets or data subsets
    \end{itemize}
    \item \textbf{Strong Models:} Individual models should perform reasonably well
\end{itemize}
\vfill
