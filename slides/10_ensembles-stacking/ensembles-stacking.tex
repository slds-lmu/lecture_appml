...
\begin{algorithm}[H]
    ...
    \For{$k \leftarrow 1$ \KwTo $K$}{
        ...
        \ForEach{$(\mathbf{x}^{(i)}, y^{(i)}) \in \D_{\mathrm{test}}^{(k)}$}{
            Add $(\mathbf{x}^{\prime(i)}, y^{(i)})$ to new dataset, where $\mathbf{x}^{\prime(i)} = (\blxi[1], \ldots, \blxi[M])$\;
        }
    }
...
\end{enumerate}
\vspace{1em}
NB: In principle we can repeat this training of multiple models at the $l$-th level indefinitely creating infinitely deep stacked ensembles
\end{vbframe}
